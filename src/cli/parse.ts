...
export const optionDeclarations: CommandLineOption[] = [
  ...
  },
  {
    name: "extension",
    description:
      'File extension for the resulting Lua files. Defaults to ".lua"',
    type: "string",
  },
  {
    name: "luaBundle",
    description:
      "The name of the lua file to bundle output lua to. Requires luaBundleEntry.",
    type: "string",
  },
  {
    name: "luaBundleEntry",
  ...
  },
  {
    name: "luaLibImport",
    description:
      "Specifies how js standard features missing in lua are imported.",
    type: "enum",
  ...
  {
    name: "disableSelfGlobal",
    description:
      "If true, treats all project files AND other files as if they were prefixed with\n/** @noSelfInFile **/.",
    type: "boolean",
  },
  {
    name: "noImplicitSelf",
  ...
  },
  {
    name: "noResolvePaths",
    description:
      "An array of paths that tstl should not resolve and keep as-is.",
    type: "array",
  },
  {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    const option = optionDeclarations.find((option) => {
      ...
      if (isShorthand && option.aliases) {
        return option.aliases.some((a) =>
          a.toLowerCase() === argumentName.toLowerCase()
        );
      }
    ...
    if (option) {
      ...
      if (consumed) {
        ...
        parsedCommandLine.fileNames = parsedCommandLine.fileNames.filter((f) =>
          f !== args[i + 1]
        );
        i += 1;
      }
...
function readValue(
  ...
  switch (option.type) {
    ...
    case "json-array-of-objects": {
      const isInvalidNonCliValue = source === OptionSource.TsConfig &&
        !Array.isArray(value);
      const isInvalidCliValue = source === OptionSource.CommandLine &&
        typeof value !== "string";
      ...
      const shouldParseValue = source === OptionSource.CommandLine &&
        typeof value === "string";
    ...
    case "enum": {
      ...
      const enumValue = option.choices.find((c) =>
