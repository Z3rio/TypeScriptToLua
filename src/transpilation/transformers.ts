...
export function getTransformers(
  ...
  const luaTransformer: ts.TransformerFactory<ts.SourceFile> =
    () => (sourceFile) => {
  ...
  const transformersFromOptions = loadTransformersFromOptions(
    program,
    diagnostics,
  );
...
export const noImplicitSelfTransformer: ts.TransformerFactory<
...
export const stripParenthesisExpressionsTransformer: ts.TransformerFactory<
  ts.SourceFile
> = (context) => (sourceFile) => {
  ...
  function unwrapParentheses(node: ts.Expression) {
    while (
      ts.isParenthesizedExpression(node) && !ts.isOptionalChain(node.expression)
    ) {
  ...
  function visit(node: ts.Node): ts.Node {
    ...
    if (ts.isCallExpression(node)) {
      ...
      return ts.factory.updateVoidExpression(
        node,
        unwrapParentheses(node.expression),
      );
    } else if (ts.isDeleteExpression(node)) {
      return ts.factory.updateDeleteExpression(
        node,
        unwrapParentheses(node.expression),
      );
    }
...
type TypeCheckerTransformerFactory = (
...
function loadTransformer(
  ...
  switch (type) {
    case "program":
      transformer = (factory as ProgramTransformerFactory)(
        program,
        extraOptions,
      );
      break;
    case "config":
      ...
      break;
    case "checker":
      transformer = (factory as TypeCheckerTransformerFactory)(
    ...
    case "compilerOptions":
      transformer = (factory as CompilerOptionsTransformerFactory)(
    ...
    default: {
      ...
      return {
        error: cliDiagnostics.argumentForOptionMustBe(optionName, "program"),
      };
    }
  }
  ...
  if (typeof transformer === "function") {
    ...
    if (after) {
      ...
      wrappedTransformer = {
        afterDeclarations: transformer as ts.TransformerFactory<
          ts.SourceFile | ts.Bundle
        >,
      };
    } else {
    ...
    const isValidGroupTransformer = typeof transformer === "object" &&
      (transformer.before ?? transformer.after ??
          transformer.afterDeclarations) !== undefined;

    if (!isValidGroupTransformer) {
      return {
        error: diagnosticFactories.transformerShouldBeATsTransformerFactory(
          transform,
        ),
      };
    }
