...
function executeCommandLine(args: string[]): void {
  if (args.length > 0 && args[0].startsWith("-")) {
    const firstOption = args[0].slice(args[0].startsWith("--") ? 2 : 1)
      .toLowerCase();
  ...
  if (
    commandLine.errors.some((e) => e.category === ts.DiagnosticCategory.Error)
  ) {
...
function performCompilation(
  ...
  const { diagnostics: transpileDiagnostics, emitSkipped } = new tstl
    .Transpiler().emit({ program });
...
function updateWatchCompilationHost(
  ...
  host.afterProgramCreate = (builderProgram) => {
    ...
    const configFileParsingDiagnostics: ts.Diagnostic[] = updateConfigFile(
      options,
    );
    ...
    if (!isBundleEnabled(options) && !hadErrorLastTime) {
      sourceFiles = [];
      while (true) {
        const currentFile = builderProgram
...
function checkNodeVersion(): void {
  ...
  if (!isValid) {
    console.error(
      `TypeScriptToLua requires Node.js >=12.13.0, the current version is ${process.version}`,
    );
    process.exit(1);
  }
}
